...
void drawOct()
{
    ...
    glRotatef(180.0, 1.0, 0.0, 0.0);
    glRotatef(360, 0.0, 1.0, 0.0);
    side(7); // сюда можно передать любой параметр int, функция side() поймёт, что это трёхцветная грань
    glPopMatrix();
    glPopMatrix();
}
...
// ------------- ФУНКЦИЯ ПЕРЕСТРОЙКИ ИЗОБРАЖЕНИЯ -----------------
...
void loadTexture()
{
    ...
    for (int i = 0; i < 9; i++)
    {
        ...
        glPixelStorei(GL_UNPACK_ALIGNMENT, 1); // указание, что выравнивание в массиве данных идёт по байту
...
void init()
{
    ...
    glEnable(GL_DEPTH_TEST); // включение теста глубины. OpenGL будет автоматически сохранять значения глубины для всех прошедших тест фрагментов и отбрасывать не прошедшие
